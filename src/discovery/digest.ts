 ...
 * outcome, in discovery order. Pure — the sender (run shell) decides when a
 * message is warranted; overlap-skipped runs never reach this builder. */
export function buildDiscoveryDigest({ run, sites }: DiscoveryDigestData): string {
  ...
  return [
    `Daily discovery run · ${run.runId.slice(0, 8)}`,
