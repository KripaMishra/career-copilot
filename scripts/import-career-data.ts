...
function safeResult(row: Row) { if (!row.safe_result) return null; const parsed = JSON.parse(String(row.safe_result)) as Record<string, unknown>; return SafeResultSchema.parse({ ...parsed, reportId: parsed.reportId ?? (row.report_id ? String(row.report_id) : null), reportPath: parsed.reportPath ?? (row.report_path ? String(row.report_path) : null), sheetReference: parsed.sheetReference ?? (row.sheet_reference ? String(row.sheet_reference) : null) }); }
...
function jobWithReport(job: Job, report: ReportImport | null): Job {
  ...
  if (!report) return { ...job, reportPath: null, sheetReference, safeResult: job.safeResult ? SafeResultSchema.parse({ ...job.safeResult, reportPath: null, sheetReference }) : null };
  return { ...job, reportId: report.reportId, reportPath: null, sheetReference, safeResult: job.safeResult ? SafeResultSchema.parse({ ...job.safeResult, reportId: report.reportId, reportPath: null, sheetReference }) : null };
}
