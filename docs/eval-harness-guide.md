# How we test an AI agent that saves your jobs
...
This document explains how we evaluate it, what the harness can and cannot prove, and what it has already caught. It is written for engineers who are wondering how to test their own agent, not just ours.
...
Honesty section, because every harness has edges:
...
The harness lives in `eval/` in the [career-copilot repository](https://github.com/KripaMishra/career-copilot), with the full specification in the repository docs and the implementation tracked in [issue #13](https://github.com/KripaMishra/career-copilot/issues/13). If you are building an eval harness for your own agent, steal the two-lane structure: a deterministic contract lane that blocks merges, and a budgeted quality lane that never overrides it.
